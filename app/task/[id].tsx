...
function TimerArc({
  ...
  return (
    <Svg width={size} height={size}>
      {bgPath ? (
        <Path d={bgPath} stroke="#2A2A2A" strokeWidth={10} fill="none" strokeLinecap="round" />
      ) : null}
      {fgPath ? (
        <Path d={fgPath} stroke={color} strokeWidth={10} fill="none" strokeLinecap="round" />
      ) : null}
      {sweepAngle > 0 ? (
        <Circle cx={dotPos.x} cy={dotPos.y} r={8} fill={color} />
      ) : null}
    </Svg>
  );
}
            ...
            <>
              ...
              <View style={styles.stepper}>
                <TouchableOpacity
                  ...
                  onPress={() => adjustPomodoro("pomodoroLength", -5)}
                >
              ...
              <View style={styles.stepper}>
                <TouchableOpacity
                  style={styles.stepperBtn}
                  onPress={() => adjustPomodoro("shortBreakLength", -1)}
                >
                  <Feather name="minus" size={18} color="#000" />
                </TouchableOpacity>
              ...
              <View style={styles.stepper}>
                <TouchableOpacity
                  style={styles.stepperBtn}
                  onPress={() => adjustPomodoro("longBreakLength", -5)}
                >
                  <Feather name="minus" size={18} color="#000" />
                </TouchableOpacity>
...
function AmbientSheet({ visible, onClose, activeSound, onSelect }: AmbientSheetProps) {
  const insets = useSafeAreaInsets();
        ...
        <View style={[styles.sheet, { paddingBottom: insets.bottom + 16 }]}>
          ...
          <View style={styles.soundGrid}>
            ...
            >
              ...
              <Text style={styles.soundLabel}>None</Text>
            </TouchableOpacity>
...
function FullscreenTimer({ visible, onClose, color, activeSound, onSoundPress }: FullscreenTimerProps) {
  const {
    isRunning,
    timeRemaining,
    phaseDuration,
  ...
  async function handleBrightnessChange(delta: number) {
    const newBrightness = Math.min(Math.max(brightness + delta, 0.05), 1);
  ...
  return (
    <Modal visible={visible} animationType="fade" transparent={false} statusBarTranslucent>
      <View style={styles.fullscreenBg}>
        <TouchableOpacity
          style={[styles.fullscreenClose, { top: (Platform.OS === "web" ? 40 : insets.top) + 8 }]}
          onPress={onClose}
        >
          <Feather name="minimize-2" size={22} color="#555" />
        </TouchableOpacity>

        <TouchableOpacity
          style={[styles.fullscreenAmbient, { top: (Platform.OS === "web" ? 40 : insets.top) + 8 }]}
          onPress={onSoundPress}
        >
          <Feather
            name="music"
            size={20}
            color={activeSound ? color : "#555"}
          />
        </TouchableOpacity>
        ...
        >
          ...
          <Text style={styles.fullscreenTime}>{formatTime(timeRemaining)}</Text>
          <Text style={styles.fullscreenElapsed}>{formatSeconds(totalSecondsSpent)}</Text>
        </Animated.View>
        ...
        <View style={[styles.fullscreenControls, { paddingBottom: (Platform.OS === "web" ? 50 : insets.bottom) + 20 }]}>
          <View style={styles.brightnessRow}>
            ...
            <Feather name="sun" size={18} color="#444" />
          </View>
...
export default function TaskScreen() {
  ...
  const insets = useSafeAreaInsets();
  const { tasks, getTodayRecord, updateTodayTime } = useTaskContext();
  const {
    activeTaskId,
    activeTask,
    isRunning,
  ...
  useEffect(() => {
    ...
    saveIntervalRef.current = setInterval(() => {
      if (isRunning) {
        updateTodayTime(task.id, totalSecondsSpent, pomodoroPartsCompleted);
      }
    }, 5000);
    return () => {
      if (saveIntervalRef.current) clearInterval(saveIntervalRef.current);
    };
  }, [task?.id, isRunning, totalSecondsSpent, pomodoroPartsCompleted]);

  useEffect(() => {
    return () => {
      if (task && totalSecondsSpent > 0) {
        updateTodayTime(task.id, totalSecondsSpent, pomodoroPartsCompleted);
      }
    };
  }, []);
  ...
  const allTimeRecords = tasks; // unused
  const targetSeconds = task.targetMinutes * 60;
  const todayProgress = targetSeconds > 0 ? Math.min((totalSecondsSpent > todaySeconds ? totalSecondsSpent : todaySeconds) / targetSeconds, 1) : 0;
  ...
  const phaseLabel =
    currentPhase === "focus"
      ...
      : "Long Break";
  const phaseColor =
    currentPhase === "focus" ? task.color : currentPhase === "shortBreak" ? "#4A90E2" : "#9B59FF";
    ...
    <View style={[styles.container, { paddingTop: topPad }]}>
      <View style={styles.topBar}>
        <TouchableOpacity
          ...
          onPress={async () => {
            await updateTodayTime(task.id, totalSecondsSpent, pomodoroPartsCompleted);
            router.back();
          }}
        >
      ...
      >
          ...
          <View style={styles.progressBarBg}>
            <View
              style={[
                ...
                { width: `${todayProgress * 100}%`, backgroundColor: task.color },
              ]}
            />
            <Text style={[styles.progressPct, { color: task.color }]}>
          ...
          <View style={styles.statsRow}>
            <View style={styles.statBox}>
              ...
              <Text style={styles.statBoxValue}>
                {formatSeconds(totalSecondsSpent > 0 ? totalSecondsSpent : todaySeconds)}
              </Text>
            </View>
            <View style={styles.statBox}>
              <Text style={styles.statBoxLabel}>Target</Text>
              <Text style={styles.statBoxValue}>{formatSeconds(targetSeconds)}</Text>
            </View>
            <View style={styles.statBox}>
        ...
        <View style={styles.timerCard}>
            ...
            <View style={styles.timerActions}>
              ...
              >
                <Feather
                  name="music"
                  size={18}
                  color={activeSound ? task.color : Colors.textSecondary}
                />
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.timerActionBtn}
          ...
          <View style={styles.timerArcContainer}>
            ...
            <View style={styles.timerCenter}>
              ...
              <Text style={styles.timerElapsed}>{formatTime(totalSecondsSpent)}</Text>
            </View>
          </View>
        ...
        <View style={styles.phaseInfoRow}>
          {["Focus", "Short Break", "Long Break"].map((label, i) => {
            const colors = [task.color, "#4A90E2", "#9B59FF"];
            const phases: typeof currentPhase[] = ["focus", "shortBreak", "longBreak"];
            const isActive = phases[i] === currentPhase;
            return (
              <View key={label} style={[styles.phaseChip, isActive && { borderColor: colors[i] }]}>
                <View style={[styles.phaseChipDot, { backgroundColor: colors[i] }]} />
                <Text style={[styles.phaseChipText, isActive && { color: colors[i] }]}>
                  {label}
                </Text>
              </View>
            );
